...
namespace Sample {
	public class HTTProxy : IDisposable {
		...
		public bool Start(List<int> ports) {
			//! HttpListenerに対応してない場合
		...
		public bool Start(int port) {
			return Start(new List<int> { port });
		}
		public bool Start(int port1, int port2) {
			return Start(new List<int> { port1, port2 });
		}
		public bool Start(int port1, int port2, int port3) {
			return Start(new List<int> { port1, port2, port3 });
		}
		...
		private void HandleHTTPRequest(HttpListenerContext ctx) {
			...
			req.UserHostAddress, req.HttpMethod, req.RawUrl, Thread.CurrentThread.ManagedThreadId); //! ここ
		...
		private HttpWebRequest CreateHttpWebRequest(HttpListenerRequest req) {
			...
			foreach(var name in req.Headers.AllKeys) {
				...
				default:
					try {
						//DebugOut(" ReqHeader[{0}]:[{1}]", name, value); //! ここ
		...
		void SetHttpListenerResponse(HttpListenerContext ctx, HttpWebResponse webResponse) {
			...
			foreach(var name in webResponse.Headers.AllKeys) {
				...
				default:
					try {
						//DebugOut(" ResHeader[{0}]:{1}", name, value);   //! ここ
		...
		private static void DebugOut(string format, params object[] args) {
			...
			string output = string.Format("HTTProxy::{0} {1}", name, string.Format(format, args));
			System.Diagnostics.Debug.WriteLine(output);
			
			SplitProxy.Form1.addTextLog(output);
		}
	}
}
